...
function OverlayManager:init(config)
    ...
    for name, OverlayClass in pairs(self.overlayClasses) do
        if OverlayClass and type(OverlayClass) == "table" and type(OverlayClass.new) == "function" then
            ...
            -- Debug: Check what's in cardDatabase for this specific overlay
            if name == "deckView" then
                print(string.format("[OverlayManager] Creating deckView overlay with cardDatabase type: %s", type(overlaySpecificConfig.cardDatabase)))
                print(string.format("[OverlayManager] Creating deckView overlay with cardDatabase value: %s", tostring(overlaySpecificConfig.cardDatabase)))
                print(string.format("[OverlayManager] Creating deckView overlay with cardDatabase.getDefinition type: %s", type(overlaySpecificConfig.cardDatabase and overlaySpecificConfig.cardDatabase.getDefinition)))
            end
