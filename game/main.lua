...
function updateCardAnimations(dt)
    ...
    for i, card in ipairs(GameState.hand) do
        if type(card) == "table" and card.isAnimating then
            local currentDuration = card.isDiscarding and discardAnimDuration or dealAnimDuration
            card.animProgress = math.min(1, (tonumber(card.animProgress) or 0) + dt / currentDuration)
            
            local t = card.animProgress
            if card.isDiscarding then t = t * t else t = t * (2 - t) end -- Ease-in for discard, Ease-out for deal
            ...
            if GameState.discardAnimCounter <= 0 then
                ...
                if numReplacements > 0 then
                    if drawCardsFromCore then
                        drawCardsFromCore(GameState, Config, Dependencies, numReplacements)
                    else
...
-- local GameOverOverlay    = require("ui.game_over_overlay") -- Example for future
...
function love.load()
    ...
    if Dependencies.cardDefinitions then
        for _, defEntry in ipairs(Dependencies.cardDefinitions) do
            if defEntry and defEntry.data and defEntry.data.id then
                Dependencies.cardDefLookup[defEntry.data.id] = defEntry.data
            end
        end
    end
    do 
 ...
 Dependencies.theme = {
    ...
    Dependencies.spellActions = {
        select = function(spellId, spellData)
    ...
    Dependencies.recipeProvider = {
        getRecipes = function() return Dependencies.potionRecipes or {} end
    }
    ...
    local overlayClassesToRegister = {
        ...
        -- gameOver      = GameOverOverlay -- Example for future
    }
    ...
    -- Calculate UI Regions and Main Screen Button Positions
    ...
    local lg, ps, pm, pl = Config.Layout.layoutGap, Config.Layout.paddingSmall, Config.Layout.paddingMedium, Config.Layout.paddingLarge 
    ...
    UIState.Regions.Goal = { x=pm, y=ps, width=sW-2*pm, height=goalConf.height or 65 }; currentY = UIState.Regions.Goal.y+UIState.Regions.Goal.height+lg
    UIState.Regions.TopBar={x=0,y=currentY,width=sW,height=(Config.UI.sortButton.height or 30)}; currentY=UIState.Regions.TopBar.y+UIState.Regions.TopBar.height+lg
    UIState.Regions.InfoArea={x=0,y=currentY,width=sW,height=(uiLineH*2)+pm}; currentY=UIState.Regions.InfoArea.y+UIState.Regions.InfoArea.height+lg
    local pAreaCfg=Config.UI.potionDisplayArea or {}; local pAreaW=sW*0.8; local pAreaX=(sW-pAreaW)/2
    UIState.Regions.PotionArea={x=pAreaX,y=currentY,width=pAreaW,height=pAreaCfg.height or 140}; currentY=UIState.Regions.PotionArea.y+UIState.Regions.PotionArea.height+lg
    local potPCfg=Config.UI.potentialPotionDisplay or {}; local potPW=potPCfg.width or 300; local potPH=potPCfg.height or 55; local potPX=(sW-potPW)/2
    UIState.Regions.PotentialPotionArea={x=potPX,y=currentY,width=potPW,height=potPH}; currentY=UIState.Regions.PotentialPotionArea.y+UIState.Regions.PotentialPotionArea.height+lg
    local btmBtnH=(Config.UI.discardButton.height or 40); local btmBtnPY=(Config.UI.discardButton.padding or pl)
    UIState.Regions.BottomBar={x=0,y=sH-btmBtnH-btmBtnPY,width=sW,height=btmBtnH}
    Config.Layout.handBaseY=currentY; UIState.Regions.HandArea={x=0,y=Config.Layout.handBaseY-(Config.Layout.handArcDepth or 0),width=sW,height=(UIState.Regions.BottomBar.y-pm)-(Config.Layout.handBaseY-(Config.Layout.handArcDepth or 0))}
    ...
    local bListX,bListY=getAnchoredPosition(topBarR.x,topBarR.y,topBarR.width,topBarR.height,bLC.width,bLC.height,0,0.5,pm,0)
    ...
    local bDeckX,bDeckY=getAnchoredPosition(topBarR.x,topBarR.y,topBarR.width,topBarR.height,bDC.width,bDC.height,1,0.5,-pm,0)
    _,Config.UI.CountInfoY=getAnchoredPosition(infoAreaR.x,infoAreaR.y,infoAreaR.width,infoAreaR.height,0,uiLineH,0,0,0,ps)
    Config.UI.ScoreInfoY=Config.UI.CountInfoY+uiLineH+ps
    Config.Animation.deckDrawX,Config.Animation.deckDrawY=getAnchoredPosition(0,0,sW,sH,Config.Card.width,Config.Card.height,1,0,-pm,pm)
    local totalBtmW=bPC.width+bSpC.width+bDiC.width+(2*pm); local startBtmX=(sW-totalBtmW)/2
    local bPotX,bPotY=startBtmX, btmBarR.y+(btmBarR.height-bPC.height)/2
    local bSpX,bSpY=bPotX+bPC.width+pm, btmBarR.y+(btmBarR.height-bSpC.height)/2
    local bDiX,bDiY=bSpX+bSpC.width+pm, btmBarR.y+(btmBarR.height-bDiC.height)/2
    ...
    if not OverlayManager:getActiveOverlayName() and GameState.gamePhase ~= "game_over" then
        ...
        if UIState.Buttons then
            for buttonId, buttonInstance in pairs(UIState.Buttons) do
                if buttonInstance and buttonInstance.update and buttonInstance.setEnabled and Dependencies.handlers.isButtonEnabled then
                    buttonInstance:update(dt, GameState.mouse.x, GameState.mouse.y, love.mouse.isDown(1))
                    local isEnabled = Dependencies.handlers.isButtonEnabled(buttonId, GameState, Dependencies) -- Pass Dependencies
        ...
        if GameState.gamePhase == "playing" and not GameState.roundCompletionPending and Dependencies.coreGame.checkStalemate then
            if Dependencies.coreGame.checkStalemate(GameState, Dependencies) then
                GameState.gamePhase = "game_over"
                GameState.gameOverReason = "stalemate" -- Example reason
...
function love.mousepressed(x, y, button, istouch, presses)
    ...
    if button == 1 then -- Primary mouse button
        ...
        if GameState.gamePhase == "game_over" then
...
function love.keypressed(key, scancode, isrepeat)
    ...
    if not (ovManager and ovManager:getActiveOverlayName()) and GameState.gamePhase == "playing" then
        if key == "s" then -- Sort hand
            if sortAPI and sortAPI.toggleSortMode then
                sortAPI.toggleSortMode(GameState, Config, Dependencies)
            end
        elseif key == "d" then -- Discard selected
            if gameHandlers and gameHandlers.isButtonEnabled and coreGameAPI and coreGameAPI.discardSelectedCards then
                if gameHandlers.isButtonEnabled("discard", GameState, Dependencies) then
                    coreGameAPI.discardSelectedCards(GameState, Config, Dependencies)
                else
                    print("Discard action not currently enabled.")
                end
            end
        elseif key == "m" then -- Make potion
            if gameHandlers and gameHandlers.isButtonEnabled and coreGameAPI and coreGameAPI.tryMakePotion then
                 if gameHandlers.isButtonEnabled("makePotion", GameState, Dependencies) then
                    coreGameAPI.tryMakePotion(GameState, Config, Dependencies)
                else
                    print("Make Potion action not currently enabled.")
                end
            end
